...
Верхнеуровневые аппаратные тесты перед измерением устанавливают основной FCLK,
...
PX-команды сначала ставят полную матрицу в кеш УПО, затем скрипт явно вызывает
`SET_PIXEL_CFG WRITE_TO_CHIP` и проверяет ответ до acquisition. Эта команда
применяется только к матричным PX-словам, а не к global DAC, EO, REF или FCLK.
В S-curve `GET_SHOT` остается в вызывающем потоке и полностью
возвращается до `GET_PIXEL`. Основной источник инжекции теперь PWM УПО:
Измерительный FCLK выставляется непосредственно перед блокирующим `GET_SHOT`,
после ответа восстанавливается основной FCLK, и лишь затем вызывается
`GET_PIXEL`. Отдельный быстрый sweep этой зависимости запускается через
`python -m comparator_characterization.high_level.run_clock_noise`.
Число фронтов для анализа рассчитывается из реальной частоты PWM и ручной
экспозиции, с сохранением неопределенности +/-1. Keysight burst остается
резервным режимом, но в `upo_pwm` VISA и внешний генератор не открываются.
