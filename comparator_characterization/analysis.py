...
def _summary_from_final(
    ...
    *,
    ...
    row = {
        ...
        "fit_selected_center_count": int((final.get("center_selected_method", pd.Series(dtype=str)) == "fit").sum()),
        "centroid_selected_center_count": int((final.get("center_selected_method", pd.Series(dtype=str)) == "weighted_centroid").sum()),
        "center_q05_v": float(centers.quantile(0.05)) if len(centers) else float("nan"),
        "center_q95_v": float(centers.quantile(0.95)) if len(centers) else float("nan"),
        "residual_abs_q90_v": float(residual.abs().quantile(0.90)) if len(residual) else float("nan"),
        "residual_abs_q95_v": float(residual.abs().quantile(0.95)) if len(residual) else float("nan"),
